...
export interface RenderingContext {
	...
	getImageData(sx: number, sy: number, sw: number, sh: number): Promise<ImageDataLike>;
	dispose(): void;
}
export declare type FramingOptions = {
	 ...
	 * ```
	 */
	resize: (frameWidth: number, frameHeight: number) => [
		renderWidth: number,
		renderHeight: number
	];
	/**
	 ...
	 * )
	 */
	crop: (renderWidth: number, renderHeight: number) => [
		cropX: number,
		cropY: number,
		cropWidth: number,
		cropHeight: number
...
export declare class Image implements Input {
	...
	[Symbol.asyncIterator](options?: InputOptions): ReturnType<Input[typeof Symbol.asyncIterator]>;
}
/**
...
export declare class Webcam implements Input {
	...
	[Symbol.asyncIterator]({ horizontalFlip, ...options }?: InputOptions): ReturnType<Input[typeof Symbol.asyncIterator]>;
	/** Turns off webcam */
	stop(): void;
}
/**
...
export interface Input {
	...
	[Symbol.asyncIterator](options?: InputOptions): AsyncGenerator<ImageDataLike>;
}
/**
...
export declare const utils: {
	createRenderingContext: (canvas?: HTMLCanvasElement | OffscreenCanvas | undefined, settings?: CanvasRenderingContext2DSettings | WebGLContextAttributes | undefined) => RenderingContext;
	createVideoElement: (source: string | Blob | MediaStream, options?: Partial<Pick<HTMLVideoElement, "loop">>) => Promise<HTMLVideoElement>;
};
...
export declare interface EmscriptenModule {
	...
	createContext: (canvas: HTMLCanvasElement, useWebGL?: boolean, setInModule?: boolean, webGLContextAttributes?: WebGLContextAttributes) => void;
...
export declare namespace BanubaSDK {
	...
	. * @param radius (float): sphere radius */
		setSphereCollider(sphereIndex: number, center: Vec3, radius: number): void;
	...
	. * @param distance (float): constaint length */
		setConstraint(bone1Name: string, bone2Name: string, distance: number): void;
	...
	. * @param material (material) sub geometry material. */
		setSubGeometryMaterial(subGeometryName: string, material: Material): void;
	...
	. * @param subGeoms (optional<Emscripten.Vector<string>>): if specified: only sub geometries from this list will be rendered. */
		addTask(layer: Layer, target: RenderTarget, subGeoms?: Emscripten.Vector<string>): number;
...
export declare namespace BanubaSDK {
	...
	export class EffectPlayerConfiguration extends EmscriptenObject {
		constructor(width: number, height: number, nnEnable: boolean, faceSearchMode: FaceSearchMode, jsDebuggerEnable: boolean, manualAudio: boolean);
	}
	export type FrameDurationListener = (instantDuration: number, averagedDuration: number) => any;
	export class FrameData extends EmscriptenObject {
		static makeFromBpc8(dataPtr: number, width: number, height: number, orientation: CameraOrientation, format: PixelFormat, requireMirroring: boolean, faceOrientation: 0): FrameData;
		getRuler(): number;
	...
	export class EffectPlayer extends EmscriptenObject {
		...
		addFrameDurationListener(recognizer: FrameDurationListener, camera: FrameDurationListener, render: FrameDurationListener): void;
		...
		pushFrameDataWithNumber(frameData: FrameData, frameNumber: number): void;
		effectManager(): EffectManager;
		setMaxFaces(maxFaces: number): void;
		setRenderConsistencyMode(mode: ConsistencyMode): void;
	}
}
declare function instantiate<T extends EmscriptenModuleOptions>(options: T): Promise<T & EmscriptenModule & typeof BanubaSDK>;
...
export declare type BanubaSDKBinary = "BanubaSDK.data" | "BanubaSDK.wasm" | "BanubaSDK.simd.wasm";
export declare type BanubaSDKBinaryFileLocator = string | ((fileName: BanubaSDKBinary) => string) | Record<BanubaSDKBinary, string> | Record<Exclude<BanubaSDKBinary, "BanubaSDK.simd.wasm">, string>;
/**
...
/** @internal */
export declare function createSDK({ clientToken, locateFile: fileLocator, canvas, logger, ...rest }: SDKOptions): Promise<EmscriptenModuleOptions & EmscriptenModule & typeof BanubaSDK>;
 ...
 * @category Core
 */
export declare class Player extends EventTarget implements globalThis.EventTarget {
	/**
...
export interface Player {
	addEventListener<PlayerEvent extends keyof PlayerEventMap>(type: PlayerEvent, listener: (this: Player, evt: CustomEvent<PlayerEventMap[PlayerEvent]>) => any, options?: boolean | AddEventListenerOptions): void;
	addEventListener(type: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions): void;
	removeEventListener<PlayerEvent extends keyof PlayerEventMap>(type: PlayerEvent, listener: (this: Player, evt: CustomEvent<PlayerEventMap[PlayerEvent]>) => any, options?: boolean | EventListenerOptions): void;
